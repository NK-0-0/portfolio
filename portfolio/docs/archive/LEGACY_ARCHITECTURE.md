> **ARCHIVED — superseded 2026-07-04.** This describes the original hover/hotspot/`SectionStore` design, including the wrong package name (`@angular-three/core` — the real package is `angular-three`). It was replaced mid-build by the scroll-driven architecture and never updated. Kept for historical record only — do not implement anything from this file. Current architecture: `docs/architecture/ARCHITECTURE.md` and `CLAUDE.md` (source of truth for AI assistants).
